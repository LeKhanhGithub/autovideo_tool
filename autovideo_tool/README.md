...
- `source_manifest.json`: provenance nguồn chính thức, URL từng chapter và SHA-256 của TXT nếu file có manifest đi kèm;
...
### 14. Lệnh đã dùng để tạo lại Global Game: AFK In The Zombie Apocalypse Game — Chapters 1–15
...
`--tts-force-refresh` bỏ qua mọi part cache hợp lệ và gọi lại Edge TTS cho toàn bộ chunk. Không dùng cờ này cùng `--tts-cache-only`.
...
### 17. Nguồn WebNovel chính thức và tải chapter theo khoảng

Các nguồn licensed đã xác minh:

- Complete Martial Arts Attributes — Henyee Translations: https://www.webnovel.com/book/complete-martial-arts-attributes_19402548505225605
- Global Game: AFK In The Zombie Apocalypse Game — Nyoi-Bo Studio: https://www.webnovel.com/book/global-game-afk-in-the-zombie-apocalypse-game_21995505805582005
- I Build a Shelter in the Eternal Night — Medieval Rabbit: https://www.webnovel.com/book/i-build-a-shelter-in-the-eternal-night_35743828100588405

`download_webnovel_range.py` chỉ tải chapter đang đọc công khai trên nguồn chính thức. Nếu WebNovel chỉ trả preview khóa, tool dừng trước khi ghi file để không thay TXT đầy đủ bằng nội dung thiếu. Mỗi lần tải thành công tạo thêm file `.source.json` chứa URL từng chapter, thời điểm tải và SHA-256. `novel_video.py` tự chép manifest này thành `source_manifest.json` trong output và ghi `source_url` vào `metadata.json`.

Lệnh đã dùng để thay file AFK Chapters 16–30 bị lẫn truyện:

```powershell
.\.venv\Scripts\python.exe download_webnovel_range.py `
  --book-url "https://www.webnovel.com/book/global-game-afk-in-the-zombie-apocalypse-game_21995505805582005" `
  --chapter-start 16 `
  --chapter-end 30 `
  --output "text_novel\Global_Game-AFK_In_The_Zombie_Apocalypse_Game\AFK In The Zombie Apocalypse Game 16-30.txt" `
  --confirm-rights
```

Sau khi tải, dùng lại lệnh ở mục 15 để tạo và thay video. Bản thay thế đã được tạo từ Chapters 16–30 chính thức, bắt đầu bằng `Chapter 16: Mutated Zombie` và kết thúc bằng `Chapter 30: The Upper Limit`.
